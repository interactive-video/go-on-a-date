...
# choice button coords [button 1: [bottomLeft, upperRight], button 2: [bottomLeft, upperRight]]
normalChooseCoords = [[[93, 359], [326, 421]], [[419, 360], [649, 422]]]
tallChooseCoords = [[[94, 338], [325, 457]], [[417, 338], [417, 459]]]
...
sceneChooseButtonChecker = (xCoord, yCoord) -> 
	...
	print chooseLeft
	print chooseRight
	print "TEST", Math.max,apply(x[0] for x in chooseLeft)
	if xCoord >= Math.min(x[0] for x in chooseLeft) and xCoord <= Math.max(x[0] for x in chooseLeft) and yCoord >= Math.min(x[1] for x in chooseLeft) and yCoord <= Math.max(x[1] for x in chooseLeft)
		print "pressed left"
	else if xCoord >= Math.min(x[0] for x in chooseRight) and xCoord <= Math.max(x[0] for x in chooseRight) and yCoord >= Math.min(x[1] for x in chooseRight) and yCoord <= Math.max(x[1] for x in chooseRight)
		print "pressed right"
	else
		print "didn't work"
