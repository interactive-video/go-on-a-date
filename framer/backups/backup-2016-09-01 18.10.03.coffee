...
sceneChooseButtonChecker = (xCoord, yCoord, currScene) -> 
	chooseLeft = chooseCoords[currScene][0]
	chooseRight = chooseCoords[currScene][1]
	if xCoord >= min(x[0] for x in chooseLeft) and xCoord <= max(x[0] for x in chooseLeft) and yCoord >= min(x[1] for x in chooseLeft) and yCoord <= max(x[1] for x in chooseLeft)
		print "pressed left"
	else if xCoord >= min(x[0] for x in chooseRight) and xCoord <= max(x[0] for x in chooseRight) and yCoord >= min(x[1] for x in chooseRight) and yCoord <= max(x[1] for x in chooseRight)
		print "pressed right"
